...
Visual merge panels generated by v1.9.3 and later (including the current v1.9.7) contain a calibrated internal 100 micrometre scale bar.
...
The latest published release is **v1.9.1** (`febcf94`, the input-scope
reachability repair), which ships its own binary and hash sidecar; the earlier
**v1.9.0** release corresponds to commit `22afada`, and the v1.9.1 commit also
archives that binary under `legacy/launchers`. The current source is
**v1.9.7**.
Version 1.9.3 retained the post-release GUI repair and added calibrated
internal scale bars, height-stabilized Step 1 and Analysis settings groups,
and the validated 20x/2k lung-field preset; the versions after it, through the
current v1.9.7, added the capabilities described above, including the Route 2
staged execution, the StarDist authority, and the isolated H&E review screen.
Building from a later `HEAD` produces a different SHA-256 because both the
launcher source and its embedded pipeline have advanced.

To build the source corresponding to a published release, build from its tag
(`22afada` for v1.9.0, `febcf94` for v1.9.1).
The resulting binary is functionally equivalent but not byte-identical: .NET
Framework `csc` output embeds a build timestamp and a fresh module version id,
so builds are not byte-reproducible. Verify a shipped binary against the
release's published `.sha256.txt` sidecar, not against a rebuild.
