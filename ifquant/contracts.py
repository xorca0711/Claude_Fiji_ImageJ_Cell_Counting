...
the runtime fail-closed subset available to route adapters. Current legacy CSV
producers do not emit this record yet; integration is an explicit roadmap gate.
Passing the checks establishes record structure, not biological validity.
"""
...
def _validate_endpoint(value: Any) -> None:
    ...
    if measured:
        ...
        expected_unit = "fraction" if numerator_unit == denominator_unit else f"{numerator_unit}_per_{denominator_unit}"
...
def _validate_sampling(value: Any) -> None:
    ...
    _exact_fields(sampling, {"design", "inclusion_probability", "selection_source"}, "sampling")
...
def _validate_channel_signature(value: Any) -> None:
    ...
    _require(indices == list(range(1, len(indices) + 1)), "channel_signature indices must be unique, ordered, and contiguous from 1")
...
def _validate_segmentation_model(value: Any) -> None:
    ...
    _require(bool(SHA256.fullmatch(str(model["model_sha256"]))), "segmentation_model.model_sha256 must be lowercase SHA-256")
...
def _validate_provenance(value: Any) -> None:
    ...
    _require(bool(SHA256.fullmatch(str(provenance["config_sha256"]))), "provenance.config_sha256 must be lowercase SHA-256")
    _require(bool(SHA256.fullmatch(str(provenance["measurement_profile_sha256"]))), "provenance.measurement_profile_sha256 must be lowercase SHA-256")
    ...
    for index, item in enumerate(inputs):
        ...
        _require(bool(SHA256.fullmatch(str(source["sha256"]))), f"provenance.inputs[{index}].sha256 must be lowercase SHA-256")
...
def validate_measurement_record(record: Mapping[str, Any]) -> None:
    ...
    _validate_endpoint(root["endpoint"])
...
def require_aggregation_eligible(record: Mapping[str, Any]) -> None:
    """Reject structurally valid records that cannot enter analytical pooling."""
